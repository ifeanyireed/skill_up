        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          {isSuper && (
            <button 
              ...
              onClick={() => {
                const headers = ['ID', 'Full Name', 'Age', 'Gender', 'Group', 'Track', 'Center', 'Parent Name', 'Parent Phone', 'Parent Email', 'Payment Status']
                const rows = children.map((c: BackendChild) => [
                  c.id, `"${c.full_name}"`, c.age, c.gender, `"${c.group}"`, `"${c.senior_track || 'N/A'}"`, `"${c.center}"`, `"${c.parent_name}"`, `"${c.parent_phone}"`, `"${c.parent_email}"`, `"${c.payment_status || 'N/A'}"`
                ])
